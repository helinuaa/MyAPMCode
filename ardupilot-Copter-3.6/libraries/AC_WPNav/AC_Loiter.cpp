...
void AC_Loiter::calc_desired_velocity(float nav_dt, float ekfGndSpdLimit)
{
    ...
    const Vector3f &desired_vel_3d = _pos_control.get_desired_velocity();  //应该是上一刻的期望速度
    ...
    if (!is_zero(desired_speed)) {
        Vector2f desired_vel_norm = desired_vel/desired_speed;  //规范化期望速度
        ...
        if (_desired_accel.is_zero()) {//只有期望加速度为0才开始计算刹车加速度
