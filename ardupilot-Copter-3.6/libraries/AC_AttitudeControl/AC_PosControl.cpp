...
{
    // compute dt
    const uint64_t now_us = AP_HAL::micros64();   //计算更新时间
    ...
    // tntranslate any adjustmes from pilot to loiter target
...
float AC_PosControl::calc_leash_length(float speed_cms, float accel_cms, float kP) const
{
    ...
    if (accel_cms <= 0.0f) {
        accel_cms = POSCONTROL_ACCELERATION_MIN;  //默认最小是50
    }

    // avoid divide by zero
    if (kP <= 0.0f) {
        return POSCONTROL_LEASH_LENGTH_MIN;  //默认最小是100
    }
    ...
    if(speed_cms <= accel_cms / kP) {
        ...
        //先从speed_cms减到accel_cms / kP，再以accel_cms / kP匀速
